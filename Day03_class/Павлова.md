...
# 1.3 Почему количество документов меньше количества вхождений? Ответ: В одном документе может быть несколько вхождений, т.е. слово встречается не один раз.
```
...
```
# 3.4 Ответ: Предполагается поиск лексем, словоформ, а не фраз.
```
...
```
__Текст___: вс   Найдено 1 810 документов, 49 511 вхождений.
__статус__: префикс Найдено 234 документа, 433 вхождения.
В результатах простого запроса со звездочкой 'вс*' "вс" - отнюдь не всегда приставка, в то время как при запросе с выбором слообразовательных признаков 'вс-' действительно является префикосм.
